...
st.set_page_config(page_title="Healthcare Test Result Prediction", page_icon="🏥", layout="wide")
...
def load_models():
    ...
    return ml_model, scaler, le

ml_model, scaler, le = load_models()
class_names = list(le.classes_)

st.title("🏥 Healthcare Test Result Prediction")
st.markdown("Patient ki details fill karo — ML model se test result predict hoga.")
st.divider()
...
with col3:
    medical_condition = st.selectbox("Medical Condition", ["Diabetes", "Hypertension", "Asthma", "Heart Disease", "Obesity", "Arthritis"])
    insurance = st.selectbox("Insurance Provider", ["Aetna", "Blue Cross", "Cigna", "Medicare", "UnitedHealthcare"])
    medication = st.selectbox("Medication", ["Aspirin", "Ibuprofen", "Lisinopril", "Metformin", "Paracetamol", "Penicillin"])
...
condition_map = {"Arthritis": 0, "Asthma": 1, "Diabetes": 2, "Heart Disease": 3, "Hypertension": 4, "Obesity": 5}
insurance_map = {"Aetna": 0, "Blue Cross": 1, "Cigna": 2, "Medicare": 3, "UnitedHealthcare": 4}
med_map = {"Aspirin": 0, "Ibuprofen": 1, "Lisinopril": 2, "Metformin": 3, "Paracetamol": 4, "Penicillin": 5}

if st.button("🔍 Predict Test Result", use_container_width=True):
    raw = np.array([[age, gender_map[gender], blood_map[blood_type],
                     condition_map[medical_condition], insurance_map[insurance],
                     billing_amount, admit_map[admission], med_map[medication]]])
